...
class XiaomiRegister {
  constructor(opts = {}) {
    ...
    this.session = new Session({ cookieFile: opts.cookieFile, proxy: opts.proxy, pool: opts.pool });
  ...
  _unwrap(r, { keepLocation = false } = {}) {
    ...
    if (j.code !== 0 && j.code !== 302 && j.code !== 70016) {
      const detail = j.desc || j.description || j.message || '';
      let hint = '';
      if (j.code === 25014) {
        // 25014 = this address is already registered on Xiaomi.
        hint = ' (address already registered)';
      } else if (j.code === 88205) {
        hint = ' (address rejected as malformed)';
      }
      const err = new Error(`xiaomi error ${j.code}: ${detail}${hint}`);
      err.code = j.code;
      err.body = j;
      throw err;
    }
