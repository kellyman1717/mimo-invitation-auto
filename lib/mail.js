...
class CloudMail {
  ...
  /**
   * Cursor marking "mail that already existed". Highest emailId currently in
   * the inbox — anything above it arrived after this call.
   */
  async baseline() {
    return Math.max(0, ...this.listAll(0, 20).map((m) => m.emailId), 0);
  }
