...
class Session {
  ...
  _curlOnce(url, o, proxy) {
    let out;
    try {
      // execFileSync only throws on a non-zero exit. curl exits 0 for any HTTP
      // status it fully received (we do not pass --fail), so exit 0 means the
      // exchange completed and anything else means it did not.
      out = execFileSync('curl', this._buildArgs(url, o, proxy),
        { maxBuffer: 64 * 1024 * 1024, encoding: 'buffer', windowsHide: true });
    } catch (e) {
      // Do NOT return the partial stdout. On a timeout curl prints the status
      // line and headers, then dies — handing that back as a response gives the
      // caller an empty body, which only surfaces much later as "Unexpected end
      // of JSON input" instead of a network error it could retry or rotate on.
      const parsed = e.stdout && e.stdout.length ? this._parse(e.stdout, o.dump) : null;
      const detail = parsed && parsed.status
        ? `HTTP ${parsed.status} then curl exited ${e.status}`
        : `curl exit ${e.status}`;
      const err = new Error(`request to ${url} failed: ${detail}`);
      err.code = e.status;
      err.status = parsed ? parsed.status : 0;
      err.partial = parsed;
      // Exit 28 (timeout) and 56 (recv failure) are usually the proxy; 6/7/35
      // and 97 are connection-level. All of them justify rotating.
